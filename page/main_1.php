				...
				<div class="col-md-4">
					<div class="form-group">
						...
						<select class="form-control" name="selectTel1">
							<option value="0">Выберите 1-й номер</option>
							
							<?php
							while ($row1 = mysqli_fetch_array($result1)){
								echo '<option value="' .$row1['id']. '">'.$row1['number_tel'].'</option>';
...
if (isset($_SERVER['REQUEST_METHOD']) && $_SERVER['REQUEST_METHOD'] == "POST"){
	...
	while($row1 = mysqli_fetch_assoc($q1)){
		$_SESSION['number_tel_1'] = $row1['number_tel']; //сохраняем в сессию номер телефона
				...
				<div class="col-md-4">
					<div class="form-group">
						...
						<select class="form-control" name="selectTel2">
							<option value="0">Выберите 2-й номер</option>
							
							<?php
							while ($row2 = mysqli_fetch_array($result2)){
								echo '<option value="' .$row2['id']. '">'.$row2['number_tel'].'</option>';
								//echo "<option value=' ".$row['id']." '>".$row['number_tel']."</option>";
							}
...
if (isset($_SERVER['REQUEST_METHOD']) && $_SERVER['REQUEST_METHOD'] == "POST"){
	...
	while($row2 = mysqli_fetch_assoc($q2)){
		$_SESSION['number_tel_2'] = $row2['number_tel']; //сохраняем в сессию номер телефона
	}
}	
				...
				<div class="col-md-4">
					<div class="form-group">
						...
						<select class="form-control" name="selectTel3">
							<option value="0">Выберите 3-й номер</option>
							
							<?php
							while ($row3 = mysqli_fetch_array($result3)){
								echo '<option value="' .$row3['id']. '">'.$row3['number_tel'].'</option>';
								//echo "<option value=' ".$row3['id']." '>".$row3['number_tel']."</option>";
							}
...
if (isset($_SERVER['REQUEST_METHOD']) && $_SERVER['REQUEST_METHOD'] == "POST"){
	...
	while($row3 = mysqli_fetch_assoc($q3)){
		$_SESSION['number_tel_3'] = $row3['number_tel']; //сохраняем в сессию номер телефона
	}
}	
